...
function GameSettingOptionsTitle({ className, ...props }: React.ComponentProps<'h1'>) {
  return (
    <h1
      className={cn(
...
function GameSettingOptions({ className, ...props }: React.ComponentProps<typeof motion.div>) {
  return (
    <motion.div
...
function GameSettingOptionsSection({
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.3 }}
      {...props}
      className={cn('flex w-full max-w-3xl flex-col items-center justify-center', className)}
    />
  )
}
